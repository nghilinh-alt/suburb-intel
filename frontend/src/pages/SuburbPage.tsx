export default function SuburbPage({ params }: { params: { id: string } }) {
  ...
  const sa2Code = params.id
  const suburbName = "Chermside QLD"
  ...
  return (
    <div style={{ padding: '40px' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
        {["Moderate retail dependency", "Rising rental pressure volatility"].map((flag, i) => (
          <span key={i} style={{ backgroundColor: '#4b566a', padding: '10px 16px', borderRadius: '6px' }}>
            ⚠️ {flag}
          </span>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px' }}>
        {["Early Growth Zone", "Infrastructure-Driven Suburb"].map((tag, i) => (
          <span key={i} style={{ backgroundColor: '#4b566a', padding: '8px 14px', borderRadius: '4px', fontSize: '14px' }}>
            {tag}
          </span>
        ))}
      </div>
